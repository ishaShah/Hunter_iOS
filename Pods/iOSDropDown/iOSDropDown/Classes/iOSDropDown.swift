...
open class DropDown : UITextField{
    var leftViewNew = UIView()

    var arrow = UIView()
    var table : UITableView!
    var shadow : UIView!
    public  var selectedIndex: Int?
 //   public  var leftViewEnabled = true
    ...
    @IBInspectable public var leftViewEnabled: Bool = true {
        didSet {
             let size = self.frame.height
                            ...
                            leftViewNew.frame =  CGRect(x: 0.0, y: 0.0, width: size, height: size)
                            self.leftView = leftViewNew
                            self.leftViewMode = .always

                            let imageV2 = UIImageView(frame: CGRect(x: 0.0, y: 7.5, width: 25, height: 25))
                           imageV2.contentMode = UIViewContentMode.scaleAspectFit
                           imageV2.image = UIImage.init(named: "search-icon")
                           leftViewNew.addSubview(imageV2)
            if leftViewEnabled == true {
                leftViewNew.isHidden = false
            }
            else if leftViewEnabled == false {
                leftViewNew.isHidden = true
...
//    @IBInspectable public var arrowColor: UIColor = .clea {
//        didSet{
...
//    }
    ...
    func setupUI () {
        ...
        let imageV = UIImageView(frame:CGRect(x: center.x - arrowSize/2, y: center.y - arrowSize/2, width: arrowSize, height: arrowSize) )
        imageV.contentMode = UIViewContentMode.scaleAspectFit
        imageV.image = UIImage.init(named: "arrow-down")
        arrowContainerView.addSubview(imageV)
...
extension DropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell!.textLabel!.text = "\(dataArray[indexPath.row])".uppercased()
        cell!.textLabel!.textColor = UIColor.init(red: 107.0/255.0, green: 62.0/255.0, blue: 153.0/255.0, alpha: 1.0)
